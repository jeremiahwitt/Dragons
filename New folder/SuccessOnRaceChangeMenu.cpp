/**
*@file SuccessOnRaceChangeMenu.cpp
*@brief Implements methods which display that the Fighter's race was updated successfully
*/
#include "SuccessOnRaceChangeMenu.h"
#include "Entity.h"

SuccessOnRaceChangeMenu::SuccessOnRaceChangeMenu(std::string title, Fighter* theFighter) : Menus(title), _theFighter(theFighter){}

void SuccessOnRaceChangeMenu::setupMenu(){
	...
	string nextDisplay = _theFighter->getName() + " is now a ";
	switch (_theFighter->getRace()){
	case Dwarf:
		nextDisplay += "Dwarf!";
		break;
	case Elf:
