...
*@brief Implementation of the AggressorStrategy class
*/
...
/**
* Parameterized Constructor. Takes the speed of the Aggressor as a parameter and passes it to the parent
* super constructor
*/
AggressorStrategy::AggressorStrategy(int speed, Characters* thePlayer) : MovementStrategy(speed), CharacterObserver(thePlayer) {}

/**
* Copy constructor, which allows conversion from Friendly Strategy to AggressorStrategy
*/
AggressorStrategy::AggressorStrategy(FriendlyStrategy *otherStrategy) : MovementStrategy(otherStrategy->getSpeed()), CharacterObserver(otherStrategy->getSubject()){
	_theMap = otherStrategy->_theMap;
	_playerVectPos = otherStrategy->_playerVectPos;
	_playerCharPos = otherStrategy->_playerCharPos;
	_vectPos = otherStrategy->_vectPos;
	_charPos = otherStrategy->_charPos;
}
...
* and will then try moving in each of those directions SPEED steps or less. Updates position after movement, and attacks the player if they are
* next to the Monster
*/
bool AggressorStrategy::move(PreBuiltLevel* level, SDL_Rect *currentGrid, GamePlayEngine* engine){
...
*/
bool AggressorStrategy::canMove(int vectI, int charI, MovementDirection dir){
...
*/
bool AggressorStrategy::_acceptableDestinationSquare(int vectI, int charI){
...
/**
* Based on the current possition that the player is at, will generate a vector with all directions the monster should try moving, from best to worst.
...
*/
vector<MovementDirection>& AggressorStrategy::_determinePlayerDirection(){
	...
	if ((vectDiff == 0) && (charDiff < 0)){
		...
		// we'll just have to do some math. Will calculate the angle between the Monster and the player
...
* will then update in the Monster so the Monster knows where the player is.
*/
void AggressorStrategy::Update(){
...
*/
bool AggressorStrategy::isWithinRange(int vectI, int charI){
